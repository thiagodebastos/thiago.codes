...
my favourites, particularly when used in combination with `filter()`, are the `some()` and
`every()` methods.
...
By understanding how these methods are implemented, you will gain an appreciation
Javascript's native array methods to write cleaner, earier to read code.
...
const sentence2AsArray = sentence1.split(" ")
